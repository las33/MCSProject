...
function n = isvaldset(a,m,c)
	...
	if c > 0 && getsize(a,3) == 0
		...
		if nargout == 0
  		error([prnewline 'Labeled dataset expected'])
		else
			n = 0;
		end
	...
	elseif getsize(a,3) < c
		...
		if nargout == 0
			error([prnewline 'Dataset should have at least ' num2str(c) ' classes'])
		else
			n = 0;
		end
	...
	if islabtype(a,'crisp') && any(classsizes(a) < m)
		...
		if nargout == 0
			if m == 1
				...
				error([prnewline 'Classes should have at least one object.' prnewline ...
						'Remove empty classes by A = remclass(A).'])
			else
				cn = num2str(m);
				error([prnewline 'Classes should have at least ' cn ' objects.' ...
						prnewline 'Remove small classes by A = remclass(A,' cn ')'])
			end
		else
			n = 0;
		end
	...
	if islabtype(a,'soft','targets') && size(a,1) < m
		error([prnewline 'Dataset should have at least ' num2str(m) ' objects'])
	end
