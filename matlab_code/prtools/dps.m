...
function [R H] = dps(A,levels,classwise)
	...
	idxs{1} = {1:size(A,1)};
  s1 = sprintf('Computing %i levels for rotation set: ',levels);
  prwaitbar(levels,s1)
	for i = 1:levels
    prwaitbar(levels,i,[s1 num2str(i)]);
    s2 = sprintf('Computing rotation set level %i',i);
    ni = 2^(i-1);
    prwaitbar(ni,s2);
		for j = 1:ni
      prwaitbar(ni,j,s2);
...
function idxs = helper(A,classwise)
  ...
  prwaitbar(size(A,1),'Density preserving sampling')
  nobj = 0;
	for i = 1:c
		...
		for j = 1:floor(m/2)
      nobj = nobj+1;
      prwaitbar(size(A,1),nobj,'Density preserving sampling')
			[mD,I] = min(D,[],1);							% \
