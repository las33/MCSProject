...
function n = isvaldfile(a,m,c)
	...
	elseif c > 0 && getsize(a,3) == 0
		...
		if nargout == 0
  		error([prnewline 'Labeled prdatafile(set) expected'])
		else
			n = 0;
		end
	...
	elseif getsize(a,3) < c
		...
		if nargout == 0
			error([prnewline 'Datafile(set) should have at least ' num2str(c) ' classes'])
		else
			n = 0;
		end
	...
	if islabtype(a,'soft','targets') && size(a,1) < m
		error([prnewline 'Datafile(set) should have at least ' num2str(m) ' objects'])
	end
