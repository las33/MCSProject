...
elseif nargin == 1
	...
	if ischar(n) && strcmpi(n,'off')           % ---- call: prwaitbar off ----
	...
	elseif ischar(n) && strcmpi(n,'on')        % ---- call: prwaitbar on ----
		STAT = 1;                             % restart waitbar
		REP = false;
    DEPTH = [];
		return
		
  elseif ischar(n) && strcmpi(n,'report')    % ---- call: prwaitbar report ----
    prwaitbar off;
    err.message = '##';
    lasterror(err);
    STAT = 1;                            
  ...
  elseif ischar(n) && strcmpi(n,'hide')      % ---- call: prwaitbar hide ----
...
elseif nargin == 2
    ...
    if (isempty(err.message)) || (~isempty(WHANDLE) && ~ishandle(WHANDLE)) | ...
            (~strcmp(err.message,'##') &&  ...
...
elseif nargin == 3
	...
	if ischar(m)                            % ---- call: prwaitbat(n,text,flag) ----
...
if ishandle(WHANDLE)
	if significant1                          % show significant updates only 
		waitbar(min(progress,1),WHANDLE,s,'position',WPOS); % update waitbar
		OLDPROG1 = progress;
		OLDPROG2 = progress;
