...
%                Default: data subdirectory of calling function
%
% DESCRIPTION
...
% datasets from PRDATAFILES and PRDATASETS. Use PR_DOWNLOAD to download
% other datasets from the web.
%
...
if nargin > 1 
  if exist(dirname,'dir') ~= 7
    ...
    if ~success
    	maindir = input(['It is not possible to create a directory here.' prnewline ...
  ...
  dirname = fullfile(callerdir,'data');
  if exist(dirname,'dir') ~= 7
    mkdir(dirname);
  end
end
...
if ~usejava('jvm') && isunix 
  ...
  if isoctave
    [dummy,status] = urlwrite(url,rfilename);
  elseif verLessThan('matlab','8')
    [dummy,status] = urlwrite(url,rfilename);
  else
    [dummy,status] = urlwrite(url,rfilename,'TimeOut',20);
  end
end
if status == 0
